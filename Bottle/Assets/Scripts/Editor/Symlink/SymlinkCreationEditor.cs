...
public static class SymlinkCreationEditor
{
    ...
    private static GUIStyle symlinkMarkerStyle
    {
        get
        {
            if (_symlinkMarkerStyle == null)
            {
                ...
                _symlinkMarkerStyle.normal.textColor = new Color(1, 0, 0, 1);
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(path))
            {
                ...
                if ((attribs & FOLDER_SYMLINK_ATTRIBS) == FOLDER_SYMLINK_ATTRIBS)
                    GUI.Label(r, "Shortcut", symlinkMarkerStyle);
            }
        }
        catch { }
    }
    ...
    /**
	 *	Add a menu item in the Bottle/Symlink category to add symlinks to directories.
	 */
#if UNITY_EDITOR_WIN
    // Create an absolute symbolic link
    [MenuItem("Bottle/Symlink/Folder (Absolute Symlink)")]
    ...
    [MenuItem("Bottle/Symlink/Folder (Relative Symlink)")]
