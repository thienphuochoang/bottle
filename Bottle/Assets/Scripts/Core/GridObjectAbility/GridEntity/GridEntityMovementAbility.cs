...
{
    [CreateAssetMenu(fileName = "GridEntityMovementAbility", menuName = "Bottle/Ability/[Grid Entity] Movement Ability", order = 1)]
    [InlineEditor]
    public class GridEntityMovementAbility : GridObjectAbilitySettings<GridEntity>
    {
        [BoxGroup("Uncontrollable Movement Settings", true, true)]
        //[HideIf("@currentGridObject.isControllable", false)]
            ...
            if (currentAssignedPathCreator != null)
            {
                if (_currentNode != currentAssignedPathCreator.nodes.Count - 1)
                {
                    if (_isMoving == false)
                    {
                        ...
                        Vector3Int stepPosGridPosition = GridManager.Instance.ConvertWorldPositionToGridPosition(_stepPos, currentGridObject.pivotOffset.y);
                        ...
                        {
                            Turn(_currentMovementDirection, GameplayManager.Instance.globalFrontDirection);
                            Vector3Int stepPosGridData = GridManager.Instance.ConvertWorldPositionToGridPosition(_stepPos, currentGridObject.pivotOffset.y);
                            Vector3Int nextNodeGridData = GridManager.Instance.ConvertWorldPositionToGridPosition(nextNodeWorldSpacePos, currentGridObject.pivotOffset.y);
        ...
        public override void AbilityStart()
        {
            ...
            if (currentGridObject.isControllable)
            {
        ...
        {
            if ((bool)message["IsControllable"] && (GridEntity)message["GridEntity"] == currentGridObject)
            {
            ...
            else if ((bool)message["IsControllable"] == false && (GridEntity)message["GridEntity"] == currentGridObject)
            {
                _movementButtonStates[KeyCode.W].ButtonDownHandler -= DetectMovementDirection;
                _movementButtonStates[KeyCode.S].ButtonDownHandler -= DetectMovementDirection;
                _movementButtonStates[KeyCode.A].ButtonDownHandler -= DetectMovementDirection;
        ...
        {
            Vector3Int targetGridPosition = new Vector3Int(currentGridObject.gridPosition.x, (int)currentGridObject.gridHeight - 1, currentGridObject.gridPosition.y) + GetValueFromDirection(theDirection, GameplayManager.Instance.globalFrontDirection);
            ...
            var (blockableGridTiles, blockableGridEntities) = GetBlockableGridObjects(currentGridObject, theDirection);
            if (targetTile.Count > 0)
            {
                if (blockableGridTiles.Count == 0 && blockableGridEntities.Count == 0)
                {
                    ...
                    {
                        Vector3Int nextTargetGridPosition = new Vector3Int(currentGridObject.gridPosition.x, (int)currentGridObject.gridHeight - 1, currentGridObject.gridPosition.y) + GetValueFromDirection(theDirection, GameplayManager.Instance.globalFrontDirection) * 2;
            ...
            {
                Vector3Int targetRampPosition = new Vector3Int(currentGridObject.gridPosition.x, (int)currentGridObject.gridHeight - 2, currentGridObject.gridPosition.y) + GetValueFromDirection(theDirection, GameplayManager.Instance.globalFrontDirection);
                ...
                if (targetRampTile.Count > 0)
                {
                    ...
                    {
                        Vector3Int nextTargetGridPosition = new Vector3Int(currentGridObject.gridPosition.x, (int)currentGridObject.gridHeight - 2, currentGridObject.gridPosition.y) + GetValueFromDirection(theDirection, GameplayManager.Instance.globalFrontDirection) * 2;
        ...
        private void Move()
        {
            ...
            Vector3 newPosition = Vector3.MoveTowards(currentGridObject.transform.position, destination, Time.deltaTime * currentSpeed);
            if (currentGridObject.transform.position != newPosition)
            {
                currentGridObject.transform.position = newPosition;
                _isMoving = true;
                GameplayManager.Instance.isTurnInProgress = true;
            }
            else
            {
                var gridPos = GridManager.Instance.ConvertWorldPositionToGridPosition(currentGridObject);
                currentGridObject.gridPosition = new Vector2Int(gridPos.x, gridPos.z);
                currentGridObject.gridHeight = gridPos.y;
        ...
        {
            Vector3 currentChildRotationEulerAngle = currentGridObject.transform.rotation.eulerAngles;
            switch (globalDirection)
            {
                case GameplayManager.GlobalDirection.POSITIVE_Z:
                    if (targetDirection == MovementDirections.FORWARD)
                    {
                        currentGridObject.transform.RotateAround(currentGridObject.transform.position, currentGridObject.transform.parent.up, 0 - currentChildRotationEulerAngle.y);
                    }
                    else if (targetDirection == MovementDirections.BACK)
                    {
                        currentGridObject.transform.RotateAround(currentGridObject.transform.position, currentGridObject.transform.parent.up, 180 - currentChildRotationEulerAngle.y);
                    }
                    else if (targetDirection == MovementDirections.LEFT)
                    {
                        currentGridObject.transform.RotateAround(currentGridObject.transform.position, currentGridObject.transform.parent.up, -90 - currentChildRotationEulerAngle.y);
                    }
